...
import { Globe, Activity, Wind, Droplets } from 'lucide-react';
...
export default function WorldMonitorPanel() {
  ...
  const [activeView, setActiveView] = useState<'earthquakes' | 'weather'>('earthquakes');

  useEffect(() => {
    const fetchData = async () => {
      const eqData = await fetchEarthquakes('day');
      setEarthquakes(eqData.map((eq: any) => ({
      ...
      setWeather(weatherData.filter(Boolean) as any[]);
    };
    ...
    <div className="space-y-4">
      <div className="terminal-panel p-4">
        ...
        <div className="flex gap-2 mt-3">
          <button
            onClick={() => setActiveView('earthquakes')}
            className={`px-3 py-1 text-xs rounded ${activeView === 'earthquakes' ? 'bg-terminal-accent text-black' : 'bg-terminal-border'}`}
          >
            Earthquakes
          </button>
          <button
            onClick={() => setActiveView('weather')}
            className={`px-3 py-1 text-xs rounded ${activeView === 'weather' ? 'bg-terminal-accent text-black' : 'bg-terminal-border'}`}
          >
            Weather
          </button>
        </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${
                    eq.mag >= 5 ? 'bg-terminal-danger/20 text-terminal-danger' :
                    eq.mag >= 4 ? 'bg-terminal-warning/20 text-terminal-warning' :
